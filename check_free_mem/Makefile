# This Makefile is for the check_free_mem extension to perl.
#
...
#     DIR => []
#     DISTNAME => q[check_free_mem]
#     EXE_FILES => [q[check_free_mem]]
#     INSTALLSCRIPT => q[/usr/lib/nagios/plugins/contrib]
#     NAME => q[check_free_mem]
#     NO_META => q[1]
#     PREREQ_PM => { Nagios::Plugin=>q[0], Getopt::Long=>q[0], English=>q[0], Pod::Usage=>q[0], version=>q[0], Nagios::Plugin::Threshold=>q[0], File::Slurp=>q[0], Carp=>q[0] }
#     VERSION => q[0.11.3]
#     dist => {  }
...
FULLEXT = check_free_mem
BASEEXT = check_free_mem
PARENT_NAME = 
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
...
TO_INST_PM = check_free_mem.pod

PM_TO_BLIB = check_free_mem.pod \
	$(INST_LIB)/check_free_mem.pod
...
DISTNAME = check_free_mem
DISTVNAME = check_free_mem-0.11.3
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  check_free_mem.pod $(INST_MAN3DIR)/check_free_mem.$(MAN3EXT) 
...
EXE_FILES = check_free_mem

pure_all :: $(INST_SCRIPT)/check_free_mem
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/check_free_mem 

$(INST_SCRIPT)/check_free_mem : check_free_mem $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/check_free_mem
	$(CP) check_free_mem $(INST_SCRIPT)/check_free_mem
	$(FIXIN) $(INST_SCRIPT)/check_free_mem
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/check_free_mem
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,11,3,0">' > $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  check_free_mem.pod $(INST_LIB)/check_free_mem.pod 
