# This Makefile is for the check_dir extension to perl.
#
...
#     DIR => []
#     DISTNAME => q[check_dir]
#     EXE_FILES => [q[check_dir]]
#     INSTALLBIN => q[/usr/lib/nagios/plugins/contrib]
#     INSTALLDIRS => q[vendor]
#     INSTALLSCRIPT => q[/usr/lib/nagios/plugins/contrib]
#     INSTALLSITEBIN => q[/usr/lib/nagios/plugins/contrib]
#     INSTALLVENDORBIN => q[/usr/lib/nagios/plugins/contrib]
#     NAME => q[check_dir]
#     NO_META => q[1]
#     PREREQ_PM => { Nagios::Plugin=>q[0], Getopt::Long=>q[0], English=>q[0], version=>q[0], Nagios::Plugin::Threshold=>q[0], File::stat=>q[0], Carp=>q[0] }
#     VERSION => q[2.0.0]
...
INSTALLVENDORBIN = /usr/lib/nagios/plugins/contrib
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  check_dir $(INST_MAN1DIR)/check_dir.$(MAN1EXT) 
...
pure_all :: $(INST_SCRIPT)/check_dir
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/check_dir 

$(INST_SCRIPT)/check_dir : check_dir $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/check_dir
	$(CP) check_dir $(INST_SCRIPT)/check_dir
	$(FIXIN) $(INST_SCRIPT)/check_dir
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/check_dir
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-stat" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
